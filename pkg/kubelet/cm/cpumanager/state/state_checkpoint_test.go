	...
	testCases := []struct {
		...
		// In below test case V2 part of checkpoint is intentionally corrupted to verify that it is not used.
		{
			...
			&stateMemory{
				...
				defaultCPUSet: cpuset.New(7, 8, 9),
				admittedEntries: ContainerCPUAdmittedEntries{
					"pod": {
						"container1": {
							AdmittedEntry: cpuset.New(4, 5, 6),
						},
						"container2": {
							AdmittedEntry: cpuset.New(1, 2, 3),
						},
					},
				},
			},
		...
		},
		{
			...
			&stateMemory{
				...
				admittedEntries: ContainerCPUAdmittedEntries{
					"pod1": {
						"container1": {
							AdmittedEntry: cpuset.New(5, 6),
						},
						"container2": {
							AdmittedEntry: cpuset.New(3, 4),
						},
					},
				},
			},
		...
		// In below testcase V2 part of checkpoint is intentionally corrupted to verify that it is not used.
		{
			...
			`{
				...
				"defaultCPUSet": "1-9",
				"checksum": 1234,
				"data": "{\"policyName\":\"static\",\"defaultCPUSet\":\"1-3\",\"entries\":{\"pod\":{\"container1\":\"4-6\",\"container2\":\"7-9\"}},\"podEntries\":{\"pod\":{\"cpuSet\":\"4-10\"}},\"admittedEntries\":{\"pod\":{\"container1\":{\"admittedEntry\":\"4-5\"},\"container2\":{\"admittedEntry\":\"7-8\"}}}}",
				"dataChecksum": 734810122
			}`,
			"static",
			containermap.ContainerMap{},
			"",
		...
		// In below testcase V2 part of checkpoint is intentionally corrupted to verify that it is not used.
		{
			...
			&stateMemory{
				...
				admittedEntries: ContainerCPUAdmittedEntries{
					"pod": {
						"container1": {
							AdmittedEntry: cpuset.New(4, 5),
						},
						"container2": {
							AdmittedEntry: cpuset.New(7, 8),
						},
					},
				},
			},
...
func TestCheckpointStateStore(t *testing.T) {
	testCases := []struct {
		...
		{
			description:    "Store admittedEntries",
		...
		t.Run(describe(fgComb), func(t *testing.T) {
			...
			for _, tc := range testCases {
				...
				t.Run(tc.description, func(t *testing.T) {
					...
					cs1.SetCPUAdmittedEntries(tc.expectedState.admittedEntries)
